...
Desde Grafana validar estos tableros:
...
Crear un deployment
```bash
kubectl create deployment [name] --image=[containerName]
```

Comandos para consultar direcciones IP de los servicios:

```bash
az network public-ip list --output table
# az network public-ip delete --resource-group microproyectoAksResourceGroup --name ...
```

Comandos para escalar los despliegues:

```bash
kubectl scale deployment/[DEPLOYMENT_NAME] --replicas=0
```

Comandos para reiniciar clúster en AKS (https://learn.microsoft.com/en-us/azure/aks/start-stop-cluster?tabs=azure-cli):

```bash
az aks stop --name microproyectoAksCluster --resource-group microproyectoAksResourceGroup
az aks show --name microproyectoAksCluster --resource-group microproyectoAksResourceGroup
```

```bash
az aks start --name microproyectoAksCluster --resource-group microproyectoAksResourceGroup
az aks show --name microproyectoAksCluster --resource-group microproyectoAksResourceGroup
```